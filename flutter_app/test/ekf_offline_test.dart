...
void main() {
  group('EkfFusionEngine6State Offline Tests', () {
    test('Propagates kinematic state with NHC and ZUPT constraints', () {
      final ekf = EkfFusionEngine6State(
          initX: 0.0, initY: 0.0, initV: 10.0, initHeading: 0.0);
      ...
      expect(
          ekf.x[3].abs(), lessThan(0.1)); // Lateral velocity constrained near 0
    ...
    test('Transitions GNSS state machine without position divergence', () {
      final ekf = EkfFusionEngine6State(
          initX: 0.0, initY: 0.0, initV: 5.0, initHeading: 0.0);
