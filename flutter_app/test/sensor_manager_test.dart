...
void main() {
  group('SensorRateTracker & QualityEngine Tests', () {
    test('Calculates actual sampling rate and jitter accurately', () {
      ...
      final diag = tracker.getDiagnostic(t,
          staleThresholdMs: 150, unavailableThresholdMs: 600);
      ...
      expect(diag.healthGrade,
          isIn([SensorHealthGrade.excellent, SensorHealthGrade.good]));
    });
    ...
    test('Transitions from LIVE to STALE to UNAVAILABLE based on age', () {
      ...
      // 50ms later -> LIVE
      final liveDiag = tracker.getDiagnostic(1050,
          staleThresholdMs: 150, unavailableThresholdMs: 600);
      expect(liveDiag.freshnessStatus, equals(SensorFreshnessStatus.live));

      // 300ms later -> STALE
      final staleDiag = tracker.getDiagnostic(1300,
          staleThresholdMs: 150, unavailableThresholdMs: 600);
      expect(staleDiag.freshnessStatus, equals(SensorFreshnessStatus.stale));

      // 1000ms later -> UNAVAILABLE
      final unavailDiag = tracker.getDiagnostic(2000,
          staleThresholdMs: 150, unavailableThresholdMs: 600);
      expect(unavailDiag.freshnessStatus,
          equals(SensorFreshnessStatus.unavailable));
    });
